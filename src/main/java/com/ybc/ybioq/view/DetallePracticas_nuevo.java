...
public class DetallePracticas_nuevo extends javax.swing.JDialog {

    private int x;
    private int y;
    DefaultTableModel model;
    public static String id_orden, afiliado, servicio, fecha, total, num_orden, hora, historia_clinica, id_ordenes;

    DefaultTableCellRenderer alinearCentro, alinearDerecha, alinearIzquierda;

    public DetallePracticas_nuevo(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
        setLocationRelativeTo(null);
        funcionescape(this);
        cargartabla(historia_clinica);
        txtOrden.setText(historia_clinica);
    ...
    private void cargartabla(String valor) {
        ...
        String sql = "SELECT ordenes_tienen_practicas.id_ordenes, practicas.codigo_practica, practicas.determinacion_practica, numero_orden,razonsocial_obrasocial,factura,Int_codigo_obrasocial,pacientes_tienen_obrasociales.numero_afiliado\n"
                ...
                + "WHERE historia_clinica.idhistoria_clinica=" + valor;
...
//                    + " WHERE id_ordenes=?";
//        try (PreparedStatement pst = cn.prepareStatement(sSQL3)){
